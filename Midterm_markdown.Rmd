...
pacman::p_load(caret, ckanr, FNN, ggcorplot, grid, gridExtra, gtools, httr, kableExtra, modelsummary, tidyverse, tidycensus, sf,  spdep, stargazer, tmap, viridis, zipcodeR)
library(ggcorrplot)
...
The following figure illustrates the spatial clustering of home prices by showing the price per finished square foot in the house. It is evident that there is clustering of higher-priced homes in and around the City of Boulder. We will need to account for this in our predictive model.
```{r}
ggplot() +
  ...
  mapTheme()
```
The following figure illustrates the correlation between house price, house age, how recently a house was improved (improvement age), and total square finished square feet. If a house has not been improved since it was built, improvement age will be 0. 
...
stargazer(data = studentData, type = "html", summary = TRUE, header = FALSE,
...
stargazer(data = reg1, type = "html", header = FALSE, title = "Regression Results")
```
...
test %>% 
  ...
  geom_abline(color = "dark green",
              size = 1) + 
  labs(title = "Predicted Price plotted on Actual Price") +
  plotTheme()
...
st_join(test, acsZipsCol.2019.sf) %>% 
...
Overall, this model is not very effective. Although the error in the model is about `r mean(test$SalePrice.APE) * 100`%, the error is large enough that if someone were to use this model, they could purchase two properties(`r paste0("$",mean(test$SalePrice.AbsError, na.rm=TRUE))`). The most interesting variables that we found were the racial categories, because they challenged our own assumptions of what the prices would be. The adjusted p-value in our model is 0.5519, which accounts for about half of the variation in the model. Our model was impacted by spatial autocorrelation, as it does appear that our geographic locations are not randomly distributed.   
Further, the model seems is better at predicting lower value homes than higher value homes. It consistently underpredicts those homes at higher actual values. Perhaps this could be remedied by gathering more data on those higher priced homes or using a method of regression other than OLS, as OLS tends to predict well on values centered around the mean, but not the outliers.
...
# Appendix 
AKA Code hoarding because we have a problem
