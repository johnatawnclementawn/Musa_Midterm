...
numericVars <- left_join(numericVars, A, by = "MUSA_ID")
...
corlistTest <- corlist %>%
  mutate(rname_name = paste(rowname, name, sep = "_"),
         name_rname = paste(name, rowname, sep = "_"))

cltest <- corlistTest %>%
  filter(!duplicated(cbind(rname_name,name_rname)))


#corlistTest[!duplicated(corlistTest[c(4,5)]),] 
