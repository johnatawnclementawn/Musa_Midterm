...
pacman::p_load(tidyverse, tidycensus, sf, kableExtra, tmap, viridis, gridExtra, spdep, caret, ckanr, FNN, grid, gridExtra, ggcorplot, httr, sf, stargazer)
...
studentData.training <- st_read("studentData.geojson", crs = CO_statePlaneCentral) %>%
  filter(toPredict == 0) %>% 
...
#Grocery.rings <-
#  st_join(st_centroid(dplyr::select(studentData.training, address)), 
#          multipleRingBuffer(Buffer_Grocery, 5280, 2640)) %>%
#  st_drop_geometry() %>%
#  left_join(dplyr::select(studentData.training, address), 
...
```{r}
BCC_houses <- studentData.training %>%
...
#filter across multiple columns https://stackoverflow.com/questions/43938863/dplyr-filter-with-condition-on-multiple-columns 

res %>%
  select(!bldgClass) %>%
  slice(-6) %>%
  filter(
    across(.cols = c(price,year),
           .fns = unique)
  )
```
...
# OLS
```{r warning=FALSE}
cor.test(BCC_houses$TotalFinishedSF, BCC_houses$price, method = "pearson")

livingReg <- lm(price ~ TotalFinishedSF, data = BCC_houses)
...
Multiple regression
```{r message=FALSE, warning=FALSE}
reg1 <- lm(price ~ ., data = st_drop_geometry(BCC_houses) %>% 
...
inTrain <- createDataPartition(
              y = paste(BCC_houses$TotalFinishedSF, boston.sf$Style, boston.sf$R_AC), 
              p = .60, list = FALSE)
