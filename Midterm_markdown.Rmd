...
numericVars <- studentData.training %>%
                  ...
                  select(!starts_with("recreation")) %>%
                  select(!starts_with("pd")) %>%
                  select(!starts_with("fd_emt")) %>%
                  select(!starts_with("hospitals")) %>%
                  select(!starts_with("malls")) %>%
                  select(!starts_with("dispensaries")) %>%
...
A <- bind_rows(
  ...
  filter(price < max(studentData.challenge$price)) %>%
  select(1,MUSA_ID, ends_with("nn1")) %>%
  select(!starts_with("flood_pts")) %>%
  mutate(nn = "nn1") %>%
  pivot_longer(cols = ends_with("nn1"), 
...
studentData.challenge <- studentData.challenge %>%
  mutate(SalePrice.Predict = predict(reg1, studentData.challenge))
