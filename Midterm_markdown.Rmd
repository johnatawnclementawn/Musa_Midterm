...
```{r Programming_School_districts, eval=FALSE, include=FALSE}
for (x in 1:nrow(acsShoolDistrctssCol.2019.sf)) {
  print(x)
  Name <- paste0('district',x)
  
district <- st_intersects(acsShoolDistrctssCol.2019.sf %>%
  slice(x),
  studentData.training)

test <- studentData.training %>%
                            mutate(Session = row_number(),
                            !!(Name) := ifelse(Session %in% district[[x]], print(Name), "NO"))
}
```


```{r hardcoding_school_distrcits, message=FALSE, warning=FALSE}
district <- st_intersects(acsShoolDistrctssCol.2019.sf %>%
  slice(1),
  studentData.training)
...
```{r Zip_codes, eval=FALSE, include=FALSE}
...
f <- studentData.training %>%
  select(starts_with("Zip"), "Session") %>%
  pivot_longer(cols = starts_with("Zip"), names_to = "remove") %>%
  filter(!value == "NO") %>%
  select(1:3)
